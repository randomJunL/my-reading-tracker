...
export function AcceptInvitationPage() {
  ...
  const { completeInvitation, isConfigured, isLoading, session, user } =
    useAuth();
  ...
  const isReaderInvitation = user?.user_metadata.account_type === "reader";

  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setError(null);
    if (!fullName.trim()) {
      setError("Enter your name to create your reader profile.");
      return;
    }
    ...
    setIsSaving(true);
    try {
      await completeInvitation({ fullName, password });
      await apiFetch<CurrentUser>("/me");
      void navigate("/", { replace: true });
    } catch (requestError) {
      setError(
        requestError instanceof Error
          ? requestError.message
          : "We couldn’t activate this invitation.",
      );
    } finally {
      setIsSaving(false);
    }
        ...
        <Card className="p-7 sm:p-9">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-[#6b7e77]">
              Enter your name and choose a password to accept the invitation.
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            >
              ...
              >
                {isSaving ? "Creating profile…" : "Accept invitation"}
              </Button>
            </form>
          ) : (
            <div className="mt-7 text-center">
              <p className="text-sm leading-6 text-[#b44733]" role="alert">
                This invitation link is invalid or has expired. Ask the account
